# -*- coding: utf-8 -*-
"""
Created on Mon Mar 11 10:07:24 2019
...
#Calculates R2 
def AccuracyScores(countyDict,predictionDict):
    CountyScores = {}
    for county in predictionDict:
        CountyScores[county] = [[],[],[],[],[],[],[],[],[]]
        for i in range(1,10):
            #r2score, RMSEscore, MAEscore = ErrorScores(CountyRateDict[county][i], predictionDict[county][i])
            #SSTotal = sum((np.array(CountyRateDict[county][i])-np.mean(CountyRateDict[county][i]))**2)
            #SSResidual = sum((np.array(CountyRateDict[county][i])- np.array(predictionDict[county][i]))**2)
            SSTotal=sum((np.array(countyDict[county][i]['count'])-np.array(predictionDict[county][i]['ybar']))**2)
            SSResidual=sum((np.array(countyDict[county][i]['count'])-np.array(predictionDict[county][i]['count']))**2)
            r2score = (SSTotal - SSResidual)/SSTotal
            scores = [r2score]
            #LR_scores = [r2score, RMSEscore, MAEscore]
            CountyScores[county][i-1] = scores 
    return CountyScores
def pltmodels():
    x=[i for i in range(1,len(countyDict['SH'][4]['count']))]    #weights from week 41 to 1
    z=[i for i in countyDict['SH'][4]['count'][1:]]
    y=[i for i in predictionkNearest['SH'][4]['count'][:-1]]   
    plt.plot(x,y,'bo-')
    plt.plot(x,z,'ro-')
    return(plot)